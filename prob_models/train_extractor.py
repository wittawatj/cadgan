...
parser.add_argument(
    ...
    type=int,
    default=1,
    help="segment input images with pretrained semantic segmentation model before feeding into G. Default=0",
)
...
parser.add_argument(
    "--annFile",
    type=str,
    default="/home/wgondal/coco_stuff/stuff_train2017.json",
    help="Provide MS-COCO Stuff Segmented Annotation File. Default=0",
)
parser.add_argument(
    "--path_data", type=str, default="/agbs/cpr/train2017/", help="path of train images. Default=ADE20K"
)
...
training_specs = "lr_{}_bs_{}__{}_{}-{}_{}{}".format(
    opt.lr, opt.batch_size, now.month, now.day, now.hour, now.minute, now.second
)
log_folder_path = os.path.join(opt.log_dir, training_specs)
writer = SummaryWriter(log_dir=log_folder_path)

print("===> Loading datasets")
train_set = Dataset_CocoSegmented(
    path=opt.path_data, cropsize=opt.cropsize, annotationFile=opt.annFile, no_of_classes=no_classes
)
...
def train(epoch):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        if iteration % 5 == 0:
            print(
                "Epoch [%d], Step[%d/%d], overall_loss: %.8f, Acc: %.4f"
